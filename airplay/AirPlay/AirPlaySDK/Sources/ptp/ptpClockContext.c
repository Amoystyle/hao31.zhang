...
double ptpClockCtxCalcmasterToLocalRatio(ClockCtxRef clock, const NanosecondTs masterTime, const NanosecondTs syncArrivalTime)
{
    ...
    if (masterTime < clock->_prevMasterTime) {
        ...
        PTP_LOG_ERROR("Negative time jump detected - inter_master_time: %lld, inter_sync_time: %lld, incorrect ppt_offset: %lf", deltaMasterTime, deltaSyncTime, freqRatio);
    ...
    *offsetValid = false;
    ...
    if (clock->_remoteClockOffsetMeasurements >= REMOTE_CLOCK_FILTER_SIZE) {
        ...
        *offsetValid = true;
        ...
        PTP_LOG_INFO("%s: _remoteClockOffsetMeasurements=%d of %d", __func__, clock->_remoteClockOffsetMeasurements, REMOTE_CLOCK_FILTER_SIZE);
    }
