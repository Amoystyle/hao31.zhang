    ...
    uint32_t inLocalSampleTime)
{
    ...
    if ((inContext->output.rtpExcessiveDriftCount >= inContext->input.rtpMaxExcessiveDriftCount) || inContext->input.forceOffsetRecalculation) {
        ...
        if (!inContext->input.forceOffsetRecalculation) {
            aps_ulog(kLogLevelNotice, "Too many samples to skew: %u\n", howManySamplesWeAreOffU32);
        }
        aps_ulog(kLogLevelTrace, "Update rtpOffsetActive: %u (%u - %u)\n",
            newRTPOffsetActive, inLocalSampleTime, rtpTimeWhereWeShouldBeU32);
