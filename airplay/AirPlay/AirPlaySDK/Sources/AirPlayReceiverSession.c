...
#define kAirTunesSetRateSkewIntervalMS 1000 // How often we'll update our skew compensation for SetRate
#define kAirTunesClockLockIntervalMS 1000 // After a PTP clock switch give the clock some time to lock.

#define kAirTunesPIDMin -50
#define kAirTunesPIDMax 50
...
OSStatus AirPlayReceiverSessionSetRateAndAnchorTime(AirPlayReceiverSessionRef inSession, const AirPlayAnchor* anchor)
{
    ...
    static const int kTimeLineMismatchMaxTries = 10; // Try 10 times

    require_action(inSession, exit, err = kNotInitializedErr);

    atr_ulog(kLogLevelNotice, "SetRateAndAnchorTime inRate %u rtpTme %u netTime (timeline=%llx secs=%d flags=%x)\n",
        anchor->rate, anchor->rtpTime,
        anchor->netTime.timelineID, anchor->netTime.secs, anchor->netTime.flags);
    ...
    if (anchor->rate > 0) {
        ...
        atr_ulog(kLogLevelNotice, "SetRateAndAnchorTime timeline %s after #%d tries\n", (err == kNoErr ? "match" : "mismatch"), tries);
